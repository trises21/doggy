...
//        }
         ...
         int loginok = 0;
        if (AutoRegister.CheckAccount(login) != false) {
            ...
            c.clearInformation();
            c.getSession().write(LoginPacket.getLoginFailed(20));
            c.getSession().write(MaplePacketCreator.serverNotice(1, "현재 회원가입이 불가능합니다."));
            return;
        }
...
//        }
        ...
        Item pHat = MapleItemInformationProvider.getInstance().getEquipById(1002505);
...
//        }
    ...
    public static final void CharSelect(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (c.getBanbyClientReason() != null && (c.isGm() || ServerConstants.Use_Localhost)) {
            ...
            c.getSession().write(MaplePacketCreator.getServerIP(1, 0, ServerConstants.Gateway_IP));
            c.getSession().close(true);
            return;
        }
        ...
        c.getSession().write(MaplePacketCreator.getServerIP(Integer.parseInt(ChannelServer.getInstance(c.getChannel()).getIP().split(":")[1]), charId, ServerConstants.Gateway_IP));
    }
