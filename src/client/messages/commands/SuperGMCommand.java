...
public class SuperGMCommand {
    ...
    public static class 스킬받기 extends CommandExecute {
    ...
    public static class 결혼 extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 3) {
                c.getPlayer().dropMessage(6, "닉네임이나 코드를 입력해주세요");
                return 0;
            }
            int itemId = Integer.parseInt(splitted[2]);
            if (!GameConstants.isEffectRing(itemId)) {
                c.getPlayer().dropMessage(6, "해당 아이템은 존재하지 않습니다.");
            } else {
                MapleCharacter fff = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
                if (fff == null) {
                    c.getPlayer().dropMessage(6, "온라인 상태가 아닙니다.");
                } else {
                    ...
                    try {
                        ...
                        for (int i = 0; i < chrz.length; i++) {
                            ...
                            chrz[i].dropMessage(6, "결혼이 성립되었습니다. 결혼상대 : " + chrz[i == 0 ? 1 : 0].getName());
                        }
    ...
    public static class 킬올2 extends CommandExecute {
...
//    }
    ...
    public static class 캐시얻기 extends CommandExecute {

        @Override
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 2) {
                c.getPlayer().dropMessage(5, "기본 문법: !캐시얻기 <숫자>");
                return 0;
            }
    ...
    public static class 메이플포인트얻기 extends CommandExecute {

        @Override
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 2) {
                c.getPlayer().dropMessage(5, "기본 문법: !메이플포인트얻기 <숫자>");
                return 0;
            }
    ...
    public static class 퀘스트몹킬 extends CommandExecute {
    ...
    public static class 퀘스트몹카운트 extends CommandExecute {
    ...
    public static class 음악바꾸기 extends CommandExecute {
    ...
    public static class TestChuseok extends CommandExecute {

        @Override
        public int execute(MapleClient c, String[] splitted) {
            OnTimeGiver.giveRandomChuseokEvent();
            return 1;
        }
    }
