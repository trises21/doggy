...
public class OnTimeGiver {
    ...
    public static void giveRandomChuseokEvent() {
        ...
        for (MapleCharacter chr : toGiveChrs) {
            chr.dropMessage(6, "온타임 이벤트에 참여되었습니다. 잠시 후, 택배함으로 이벤트 상품이 지급될 예정입니다.");
        }

        for (MapleCharacter chr : toGiveChrs) {
            if (chr != null && chr.getClient() != null) {
                try {
                    giveItemByParcel(1002430, 1, chr);
                    giveItemByParcel(5121004, 50, chr);
                    giveItemByParcel(getRandomChair(), 1, chr);
                    giveItemByParcel(4001101, 20, chr);
                    chr.getClient().sendPacket(MaplePacketCreator.receiveParcel("[온타임이벤트]", true));
                    chr.dropMessage(6, "온타임 이벤트 상품이 도착하였습니다. 택배함에서 상품을 확인하세요.");
                } catch (Exception e) {
                }
            }
        }
    ...
    private static void giveItemByParcel(int itemid, int quantity, MapleCharacter chr) {
        DueyHandler.addNewItemToDb(itemid, quantity, chr.getId(), "[이벤트]", "화이트스타가 어느덧 1주년이 되어갑니다. 모두 여러분 덕분입니다. 그동안 감사했습니다.", true);
