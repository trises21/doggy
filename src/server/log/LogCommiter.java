...
//            commitSchedule = Timer.CloneTimer.getInstance().register(timer, interval);
//        }
        file = new File("sql/vacon.sql");
        try {
            fos = new FileOutputStream(file);
            fos.write(("-- VaconMaple Log SQL -- " + System.getProperty("line.separator") + System.getProperty("line.separator")).getBytes(Charset.forName("MS949")));
        } catch (FileNotFoundException ex) {
            Logger.getLogger(LogCommiter.class.getName()).log(Level.SEVERE, null, ex);
        } catch (IOException ex) {
            Logger.getLogger(LogCommiter.class.getName()).log(Level.SEVERE, null, ex);
        }
        System.out.println("[VaconMaple] Log Commiter Initialized.");
    }
