...
import static server.ServerProperties.loadProperties;
import server.Start;
...
public class ChannelServer {
    ...
    public int expRate, mesoRate, dropRate, cashRate = 3, traitRate = 3;
    private short port;
    ...
    public final void run_startup_configurations() throws Exception {
     ...
     try {       
      this.expRate = ServerConstants.defaultExpRate;
      this.mesoRate = ServerConstants.defaultMesoRate;
      this.dropRate = ServerConstants.defaultDropRate;
      this.serverMessage = ServerConstants.serverMessage;

      this.flags = ServerConstants.defaultFlag;
      
      this.serverName = ServerConstants.serverName;

      this.eventSM = new EventScriptManager(this, ServerProperties.getProperty("net.sf.odinms.channel.events").split(","));

      this.port = (short)(ServerConstants.basePorts + getChannel());
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
    this.ip = (ServerConstants.Gateway_IP + ":" + this.port);
    
        System.out.println("[VaconMaple] " + channel + " Channel Open");
