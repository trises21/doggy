...
public class MemoryUsageWatcher extends Thread {
    ...
    public MemoryUsageWatcher(int rebootPercent) {
        ...
        System.out.println("[VaconMaple] Memory Usage Watcher Started. Min Reboot Usage : " + minRebootUsage / 1024 + "K");
    }
    ...
    public void run() {
        ...
        while (!overflow) {
            try {
                MemoryUsage mem = mmb.getHeapMemoryUsage();
                System.out.println("[VaconMaple] By. tjsdn9666@naver.com - Online : " + ChannelServer.getOnlineConnections() + " (Memory : " + mem.getUsed() / 1024 + "K/" + mem.getMax() / 1024 + "K)");
